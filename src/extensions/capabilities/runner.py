...
class CapabilityRunner:
    ...
    def handle(
        ...
        if response.completed:
            ...
            # This is the source of truth for capability completion state
            if response.facts is None:
                logger.warning(
                    f"Adapter {capability} completed but returned facts=None. "
                    f"Adapters must emit completion facts (e.g., payment_satisfied: True). "
                    f"Using empty dict as fallback."
                )
                facts = {}
            elif not isinstance(response.facts, dict):
                logger.warning(
                    f"Adapter {capability} completed but returned invalid facts type: {type(response.facts)}. "
                    f"Expected dict, got {type(response.facts)}. Using empty dict as fallback."
                )
                facts = {}
            else:
            ...
            if facts:
                logger.info(
                    f"Capability {capability} completed with facts: {list(facts.keys())}"
                )
            else:
                logger.warning(
                    f"Capability {capability} completed with empty facts dict. "
                    f"This may prevent capability gating from recognizing completion."
                )
            ...
            logger.error(
                f"[PAYMENT_CAPABILITY_DEBUG] Returning RunnerResult for capability={capability}, user={user_id}: "
                f"passthrough=True, facts={facts}, facts_type={type(facts)}, "
                f"facts_is_none={facts is None}, facts_keys={list(facts.keys()) if isinstance(facts, dict) else 'N/A'}, "
                f"active_capability=None"
            )

            # Return result indicating completion
            # CRITICAL: facts must be a dict (never None) when adapter completes
            result = RunnerResult(
            ...
            logger.error(
                f"[PAYMENT_CAPABILITY_DEBUG] RunnerResult constructed: "
                f"passthrough={result.passthrough}, "
                f"facts={result.facts}, facts_type={type(result.facts)}, "
                f"facts_is_none={result.facts is None}, "
            ...
            logger.error(
                f"[PAYMENT_CAPABILITY_DEBUG] Adapter still active for capability={capability}, user={user_id}: "
                f"response.completed={response.completed}, response.facts={response.facts}, "
                f"returning RunnerResult(passthrough=False, active_capability={capability})"
            )
            ...
            logger.error(
                f"[PAYMENT_CAPABILITY_DEBUG] RunnerResult (non-completion) constructed: "
                f"passthrough={result.passthrough}, facts={result.facts}, "
                f"active_capability={result.active_capability}"
            )
