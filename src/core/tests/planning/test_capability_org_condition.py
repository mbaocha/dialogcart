...
def test_org_payment_required_false_no_blocking():
    ...
    luma_response = {
        ...
        "facts": {
            "org": {"payment_required": False},  # Payment not required
    ...
    assert (
        plan.get("active_capability") is None
...
def test_org_payment_required_true_blocks():
    ...
    mock_luma_response = {
        ...
        "missing_slots": [],
        "needs_clarification": False,
        "facts": {"payment_satisfied": False},  # Payment not satisfied
    }
    ...
    assert awaiting == "PAYMENT", f"awaiting should be PAYMENT, got: {awaiting}"
...
def test_condition_evaluator_org_namespace():
    ...
    """
    facts = {"org": {"payment_required": True}}
    slots = {}
    session_state = None
    ...
    assert (
        result is False
    ), "org.payment_required == false should evaluate to False when compared to true"
...
def test_condition_evaluator_org_from_session_state():
    ...
    slots = {}
    session_state = {"org": {"payment_required": True}}
