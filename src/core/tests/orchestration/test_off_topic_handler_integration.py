"""
Integration tests: OFF_TOPIC Core digression path.

Covers:
- Cold start OFF_TOPIC → Core OFF_TOPIC outcome (no extension handler)
...
class TestOffTopicHandlerIntegration:
    ...
    def test_cold_start_off_topic_core_digression_without_booking_planner(self):
        luma = Mock(spec=LumaClient)
        luma.resolve.return_value = _off_topic_luma_payload()

        with patch(
            "core.rendering.off_topic_renderer.render_llm",
            return_value="Tinubu is president. How can I help you today?",
        ):
        ...
        assert outcome.get("status") == "OFF_TOPIC"
        assert outcome.get("active_handler") in (None, "")
        assert outcome.get("intent_name") == "OFF_TOPIC"
        assert outcome.get("search_query") in (None, "")
        assert outcome.get("answerable") is True
        assert "Tinubu" in (outcome.get("answer") or "")
    ...
    def test_off_topic_renderer_uses_stage2_evidence_not_faq(self):
        ...
        assert req.facts.get("scope") == "off_topic"
        assert req.facts.get("booking_active") is False
        assert req.facts.get("answerable") is True
        assert "Tinubu" in (req.facts.get("answer") or "")
        assert "facts first" in req.render_instruction.lower()
        assert "resume" in req.render_instruction.lower()
        assert "evidence" not in req.render_instruction.lower()

        booking_req = build_off_topic_render_request(
            {
                "off_topic_query": "Who is the president of Nigeria?",
                "answerable": True,
                "answer": "Nigeria's current president is Bola Ahmed Tinubu.",
        ...
        assert booking_req.facts.get("booking_active") is True
        assert booking_req.render_instruction == req.render_instruction

        decline = build_off_topic_render_request(
            {
        ...
        assert decline.facts.get("answerable") is False
        assert "misunderstood" in decline.render_instruction.lower()
        assert "decline" in decline.render_instruction.lower()
    ...
    def test_mid_booking_off_topic_preserves_session(self):
        ...
        with patch(
            "core.rendering.off_topic_renderer.render_llm",
            return_value="Let's continue your booking",
        ):
            result = handle_message(
                text="Who is the president of Nigeria?",
                user_id=user_id,
