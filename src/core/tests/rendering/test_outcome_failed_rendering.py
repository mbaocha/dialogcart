...
def test_failed_with_specific_intent_uses_intent_specific_template():
    ...
    decision = {"intent_name": "CREATE_APPOINTMENT"}
    outcome = {"status": "FAILED", "intent_name": "CREATE_APPOINTMENT"}
    ...
    assert (
        "couldn't" in render_spec.text.lower()
        or "could not" in render_spec.text.lower()
    )
...
def test_failed_unknown_intent_falls_back_to_generic():
    ...
    decision = None
    outcome = {"status": "FAILED", "intent_name": "UNKNOWN_INTENT"}
    ...
    assert (
        "wrong" in render_spec.text.lower() or "try again" in render_spec.text.lower()
    )
...
def test_failed_with_no_templates_returns_none():
    ...
    decision = {"intent_name": "UNKNOWN_INTENT"}
    outcome = {"status": "FAILED", "intent_name": "UNKNOWN_INTENT"}
    ...
    assert render_spec is None or (
        render_spec is not None and render_spec.text is not None
    )
