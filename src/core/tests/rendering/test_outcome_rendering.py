...
from core.rendering.mapper.outcome_mapper import extract_intent, derive_outcome_template_key_candidates
...
def test_render_executed_cancel_booking():
    ...
    assert render_spec is not None
    assert render_spec.text is not None
    assert "cancelled" in render_spec.text.lower() or "cancel" in render_spec.text.lower()
