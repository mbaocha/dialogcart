...
def test_payment_capability_renders_template_text():
    ...
    with patch('core.rendering.capability_renderer.get_adapter', return_value=mock_payment_adapter):
        facts = {}
        slots = {"booking_code": "ABC123"}
        context = {
            "session_slots": {"booking_code": "ABC123"},
            "session_facts": {}
        }
...
def test_render_capability_template_missing_template_falls_back():
    ...
    with patch('core.rendering.capability_renderer.get_adapter', return_value=mock_payment_adapter):
        ...
        with patch('core.rendering.capability_renderer._load_capability_templates', side_effect=FileNotFoundError):
            facts = {}
            slots = {"booking_code": "ABC123"}
            context = {
                "session_slots": {"booking_code": "ABC123"},
...
def test_render_capability_payment_with_template():
    ...
    with patch('core.rendering.capability_renderer.get_adapter', return_value=mock_payment_adapter):
        ...
        render_spec = render_capability(status, active_capability, facts, slots, context)
