...
def test_missing_template_raises_keyerror():
    ...
    assert (
        "UNKNOWN_REASON_THAT_DOES_NOT_EXIST" in error_message
    ), f"Expected error message to mention the unknown reason, got: {error_message}"
    assert (
        "template" in error_message.lower() or "not found" in error_message.lower()
...
def test_missing_required_fields_raises_valueerror():
    ...
    error_message = str(exc_info.value)
    assert (
        "service" in error_message.lower() or "required" in error_message.lower()
