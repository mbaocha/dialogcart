...
def test_adaptive_clarification_variant_after_retry():
    ...
    assert status1 == "NEEDS_CLARIFICATION", \
    ...
    assert isinstance(slot_attempts_session1, dict), \
        f"slot_attempts should be a dict, got {type(slot_attempts_session1)}"
    assert slot_attempts_session1.get("date") == 1, \
    ...
    facts1_decision = decision1.get("facts", {}) if decision1 else outcome1.get("facts", {})
    slot_attempts_decision1 = facts1_decision.get("slot_attempts", {})
    assert isinstance(slot_attempts_decision1, dict), \
        f"slot_attempts should be a dict in decision.facts, got {type(slot_attempts_decision1)}"
    assert slot_attempts_decision1.get("date") == 1, \
    ...
    assert status2 == "NEEDS_CLARIFICATION", \
    ...
    if decision2:
        ...
        assert plan_status2 == "NEEDS_CLARIFICATION", \
    ...
    assert isinstance(missing_slots2, list), \
        f"missing_slots should be a list, got {type(missing_slots2)}"
    assert "date" in missing_slots2, \
