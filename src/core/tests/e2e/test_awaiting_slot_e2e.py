...
def test_e2e_awaiting_slot_soft_guided_flow():
    ...
    assert saved_session.get("awaiting_slot") == "service_id", \
    ...
    assert "service_id" in missing_slots_turn1, \
    ...
    assert missing_slots_turn1[0] == "service_id", \
    ...
    assert session_after_turn1.get("awaiting_slot") == "service_id", \
    ...
    assert session_before_turn2.get("status") == "NEEDS_CLARIFICATION", \
        f"Expected status=NEEDS_CLARIFICATION before turn 2 (so session is loaded), got {session_before_turn2.get('status')}"
    assert session_before_turn2.get("awaiting_slot") == "service_id", \
    ...
    assert "service_id" not in missing_slots_turn2, \
    ...
    assert status_turn2 in ("READY", "NEEDS_CLARIFICATION", "AWAITING_CONFIRMATION"), \
    ...
    if session_after_turn2 is not None:
        ...
        assert awaiting_slot_after_turn2 is None, \
