"""
End-to-End Test: Multi-Turn Modification Application with Real Luma

Tests the full flow: planning → session persistence → execution for booking modification
...
- Modification execution (APPLY_MODIFICATION) after booking_id and modification fields are supplied
- Booking client integration
- Session clearing after confirmation

To run this test:
  RUN_REAL_LUMA_E2E=true pytest core/tests/e2e/test_core_e2e_followup_apply_modification_real_luma.py

To run a specific test by index (0-based):
  E2E_TEST_INDEX=0 RUN_REAL_LUMA_E2E=true pytest core/tests/e2e/test_core_e2e_followup_apply_modification_real_luma.py
...
Note:
  This test is expected to FAIL initially because APPLY_MODIFICATION is not yet
...
def load_scenarios() -> List[Dict[str, Any]]:
    ...
    scenarios_file = (
        ...
        / "scenarios"
        / "followup_apply_modification_real_luma.yaml"
    )
...
def create_mock_booking_client() -> Mock:
    ...
    # Mock update_booking method for modification tests
    def mock_update_booking(organization_id=None, booking_id=None, **kwargs):
        return {"success": True, "booking_id": booking_id, "status": "modified"}

    mock_booking_client.update_booking = Mock(side_effect=mock_update_booking)
...
def assert_turn_expectations(
    ...
    # Assert no premature execution
    if expectations.get("no_premature_execution"):
        actual_action = plan.get("action")
        assert (
            actual_action != "APPLY_MODIFICATION"
        ), f"[{scenario_name}] Turn {turn_number}: Expected no premature APPLY_MODIFICATION, got {actual_action}"
...
def test_real_luma_followup_apply_modification_scenario(scenario: Dict[str, Any]):
    """
    ...
    for turn_idx, turn in enumerate(turns, start=1):
        ...
        # Check if this turn expects APPLY_MODIFICATION action
        # NOTE: Dispatcher does not yet support APPLY_MODIFICATION execution.
        # This test only verifies that the planner selects APPLY_MODIFICATION action
        # after availability_resolved=True (from previous SEARCH_AVAILABILITY execution).
        expected_action = expectations.get("action")
        if expected_action == "APPLY_MODIFICATION":
            # Only assert action selection (planning), not execution
            # Execution assertions will be added when dispatcher supports APPLY_MODIFICATION
            pass
        ...
        if turn_idx < len(turns):  # Not the last turn
            ...
            if expected_action == "SEARCH_AVAILABILITY":
                ...
                if (
                    ...
                    and execution_result.get("type") == "availability"
                ):
