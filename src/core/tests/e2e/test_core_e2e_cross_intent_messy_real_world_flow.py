"""
End-to-End Sentinel Test: Cross-Intent Messy Real-World Flow

Prefer: RUN_REAL_LUMA_E2E=true python core/tests/test.py --category smoke
Targeted: RUN_REAL_LUMA_E2E=true pytest core/tests/e2e/test_core_e2e_cross_intent_messy_real_world_flow.py
"""

import os
import sys
from pathlib import Path
from typing import Any, Dict

import pytest

src_path = Path(__file__).parent.parent.parent.parent
if str(src_path) not in sys.path:
    sys.path.insert(0, str(src_path))

if not os.getenv("RUN_REAL_LUMA_E2E"):
    pytest.skip(
        "Real Luma E2E tests disabled. Set RUN_REAL_LUMA_E2E=true to enable.",
        allow_module_level=True,
    )

from core.tests.harness.legacy_e2e import load_e2e_scenarios, run_legacy_e2e_scenario
from core.tests.harness.scenario_loader import scenario_param_id

_SCENARIOS = load_e2e_scenarios(
    Path(__file__).parent / "scenarios", "cross_intent_messy_real_world_flow.yaml"
)


@pytest.mark.parametrize(
    "scenario",
    _SCENARIOS,
    ids=[scenario_param_id(s, _SCENARIOS) for s in _SCENARIOS],
)
def test_cross_intent_messy_real_world_flow(scenario: Dict[str, Any]):
    run_legacy_e2e_scenario(scenario, user_id_prefix="test_cross_intent")
