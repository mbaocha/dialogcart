...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    ...
    if failing_indices:
        ...
        for idx in sorted(failing_indices):
            ...
            test_file = (
                "core/tests/e2e/test_core_e2e_followup_availability_real_luma.py"
            )
            print(
                f"  E2E_TEST_INDEX={idx} RUN_REAL_LUMA_E2E=true pytest --tb=short {test_file}  # {scenario_name}"
            )
        ...
        if len(failing_indices) > 1:
            ...
            print(
                f"  E2E_TEST_INDEX={indices_str} RUN_REAL_LUMA_E2E=true pytest --tb=short {test_file}"
            )
