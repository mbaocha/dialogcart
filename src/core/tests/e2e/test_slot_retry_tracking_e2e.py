...
def test_e2e_slot_retry_tracking_increment_and_reset():
    ...
    assert result_turn1.get("success"), f"Turn 1 should succeed, got: {result_turn1.get('error')}"
    ...
    assert status_turn1 == "NEEDS_CLARIFICATION", \
    ...
    assert isinstance(missing_slots_turn1, list), \
        f"missing_slots should be a list, got {type(missing_slots_turn1)}"
    assert "date" in missing_slots_turn1, \
    ...
    assert missing_slots_turn1[0] == "date", \
    ...
    assert session_after_turn1.get("status") == "NEEDS_CLARIFICATION", \
        f"Expected status=NEEDS_CLARIFICATION after turn 1, got {session_after_turn1.get('status')}"
    assert "slot_attempts" in session_after_turn1, \
    ...
    facts_turn1 = decision_turn1.get("facts", {}) if decision_turn1 else outcome_turn1.get("facts", {})
    slot_attempts_decision_turn1 = facts_turn1.get("slot_attempts", {})
    assert isinstance(slot_attempts_decision_turn1, dict), \
        f"slot_attempts should be a dict in decision.facts, got {type(slot_attempts_decision_turn1)}"
    assert slot_attempts_decision_turn1.get("date") == 1, \
    ...
    assert session_before_turn2.get("slot_attempts", {}).get("date") == 1, \
    ...
    assert result_turn2.get("success"), f"Turn 2 should succeed, got: {result_turn2.get('error')}"
    ...
    assert status_turn2 == "NEEDS_CLARIFICATION", \
    ...
    assert slot_attempts_session_turn2.get("date") == 2, \
        f"Expected slot_attempts['date'] == 2 in session, got {slot_attempts_session_turn2.get('date')}"
    assert "time" not in slot_attempts_session_turn2, \
    ...
    facts_turn2 = decision_turn2.get("facts", {}) if decision_turn2 else outcome_turn2.get("facts", {})
    slot_attempts_decision_turn2 = facts_turn2.get("slot_attempts", {})
    assert slot_attempts_decision_turn2.get("date") == 2, \
    ...
    assert result_turn3.get("success"), f"Turn 3 should succeed, got: {result_turn3.get('error')}"
    ...
    assert status_turn3 == "NEEDS_CLARIFICATION", \
    ...
    facts_turn3 = decision_turn3.get("facts", {}) if decision_turn3 else outcome_turn3.get("facts", {})
    slot_attempts_decision_turn3 = facts_turn3.get("slot_attempts", {})
    assert "date" not in slot_attempts_decision_turn3, \
        f"Expected 'date' NOT in slot_attempts in decision.facts, got {list(slot_attempts_decision_turn3.keys())}"
    assert slot_attempts_decision_turn3.get("time") == 1, \
    ...
    assert result_turn4.get("success"), f"Turn 4 should succeed, got: {result_turn4.get('error')}"
    ...
    assert isinstance(missing_slots_turn4, list), \
        f"missing_slots should be a list, got {type(missing_slots_turn4)}"
    assert len(missing_slots_turn4) == 0, \
    ...
    if session_after_turn4 is not None:
        ...
        if slot_attempts_session_turn4:
            ...
            assert len(slot_attempts_session_turn4) == 0, \
    ...
    assert outcome_turn1.get("intent_name") == "CREATE_APPOINTMENT" or outcome_turn1.get("intent") == "CREATE_APPOINTMENT", \
        "Intent should be CREATE_APPOINTMENT"
    assert outcome_turn4.get("intent_name") == "CREATE_APPOINTMENT" or outcome_turn4.get("intent") == "CREATE_APPOINTMENT", \
    ...
    assert "service_id" in slots_turn4 or "service" in slots_turn4, \
        "Service slot should be collected"
    assert "date" in slots_turn4, \
        "Date slot should be collected"
    assert "time" in slots_turn4, \
        "Time slot should be collected"
