...
Tests for core.execution.confirmation.confirm_booking()
...
def test_confirm_booking_happy_path():
    ...
    mock_response = {"booking": {"booking_code": "BK123", "status": "confirmed"}}
    mock_client.confirm_booking.return_value = mock_response

    # Prepare input slots
    slots = {"organization_id": 1, "booking_code": "BK123"}

    # Call client
    result = mock_client.confirm_booking(booking_code="BK123", organization_id=1)
    ...
    expected_core_result = {
        "type": "confirmation",
        "status": "success",
        "booking": {"booking_code": "BK123", "status": "confirmed"},
    }
...
def test_confirm_booking_invalid_state():
    ...
    try:
        mock_client.confirm_booking(booking_code="BK123", organization_id=1)
    ...
    try:
        mock_client2.confirm_booking(booking_code="BK456", organization_id=1)
