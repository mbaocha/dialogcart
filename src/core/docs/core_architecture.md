...
| 3 | Plan | `orchestrator.plan_message(...)` → `planning/planner/turn_planner.plan_turn(..., planning_only=True)` | Full planning pipeline (below). |
...
**File:** `planning/planner/turn_planner.py` — `plan_turn()`.
...
| Change NLU→plan pipeline | `planning/planner/turn_planner.py` |
...
| Change how status/action are derived | `planning/planner/plan_builder.py` |
