...
from .invariants import _is_bare_weekday_range, _slot_present, _validate_temporal_shape
...
def compute_missing_slots(
        ...
        if isinstance(aliases, dict) and aliases:
            ...
            if resolved_tenant_service_id not in aliases:
                ...
                for alias_key, canonical_value in aliases.items():
                    ...
                    if isinstance(resolved_tenant_service_id, str) and isinstance(
                        canonical_value, str
                    ):
                        if (
                            "." in resolved_tenant_service_id
                            and resolved_tenant_service_id.endswith(
                                f".{canonical_value}"
                            )
                        ):
                            is_canonical_value = True
    ...
    if temporal_shape == APPOINTMENT_TEMPORAL_TYPE:
        ...
        temporal_issues = _validate_temporal_shape(
            temporal_shape, merged_state, intent_meta
        )
