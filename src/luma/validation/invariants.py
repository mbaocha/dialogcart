...
class ValidationResult:
    ...
    clarification_reason: Optional[str] = (
        None  # Clarification reason code if NEEDS_CLARIFICATION
    )
...
def compute_invariants(
    intent_name: str, semantic_slots: Dict[str, Any], intent_meta: Optional[IntentMeta]
) -> ValidationResult:
    """
    ...
    if not intent_meta:
        return ValidationResult(status=STATUS_READY, issues={}, missing_slots=[])
    ...
    if temporal_shape:
        temporal_issues = _validate_temporal_shape(
            temporal_shape, semantic_slots, intent_meta
        )
    ...
    if status == STATUS_NEEDS_CLARIFICATION:
        clarification_reason = _derive_clarification_reason(
            issues, intent_name, semantic_slots, intent_meta
        )
        ...
        if services and len(services) > 0:
            ...
            for service in services:
                if isinstance(service, dict):
                    if (
                        service.get("tenant_service_id")
                        or service.get("family_id")
                        or service.get("text")
                    ):
                        return True
                elif service:
...
def _validate_temporal_shape(
    temporal_shape: str, semantic_slots: Dict[str, Any], intent_meta: IntentMeta
) -> Dict[str, str]:
    """
    ...
    if temporal_shape == APPOINTMENT_TEMPORAL_TYPE:
        ...
        # Luma is stateless - all data comes from the current request
        has_valid_date = len(date_refs) > 0 or has_bound_date
        ...
        has_valid_time = (
            time_constraint is not None
            and time_constraint.get("mode")
            in {TimeMode.EXACT.value, TimeMode.WINDOW.value, TimeMode.FUZZY.value}
        ) or (
            time_mode
            in {
                TimeMode.EXACT.value,
        ...
        if not ALLOW_BARE_WEEKDAY_RANGE_BINDING:
            ...
            if (
                date_mode == "range"
                and len(date_refs) >= 2
                and _is_bare_weekday_range(date_refs, date_modifiers)
            ):
        ...
        has_start = (len(date_refs) >= 1 and date_mode not in ("none", "flexible")) or (
            has_bound_date and date_range
        )  # date_range implies start_date
        ...
        if requires_end_date:
            has_end = (
                (len(date_refs) >= 2 and date_mode not in ("none", "flexible"))
                or (date_mode == "range")
                or (
                    has_bound_date and date_range and date_range.get("end")
                )  # date_range with end implies end_date
            )
            if not has_end:
                issues["end_date"] = "missing"
...
def _derive_clarification_reason(
    ...
    if intent_meta and intent_meta.temporal_shape == RESERVATION_TEMPORAL_TYPE:
        ...
        if "start_date" in issues:
            return (
                ClarificationReason.MISSING_DATE.value
            )  # Use MISSING_DATE for start_date
        if "end_date" in issues:
            return (
                ClarificationReason.MISSING_DATE.value
            )  # Use MISSING_DATE for end_date
