...
spec_norm = importlib.util.spec_from_file_location(
    "luma.extraction.normalization", normalization_path
)
...
spec_loading = importlib.util.spec_from_file_location(
    "luma.extraction.entity_loading", entity_loading_path
)
...
spec_processing = importlib.util.spec_from_file_location(
    "luma.extraction.entity_processing", entity_processing_path
)
...
def print_date_result(doc, result: Dict[str, List], test_input: str):
    ...
    if dates:
        ...
        for i, date in enumerate(dates, 1):
            ...
            print(
                f"   {i}. '{text}' [pos: {position}, len: {length}, span: {start}-{end}]"
            )
...
def main():
    ...
    test_cases = [
        ...
        (
            "schedule on 15/13/2025",
            "Malformed: invalid month (may still match pattern)",
        ),
