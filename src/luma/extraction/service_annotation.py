...
def annotate_service_tokens(
    ...
    alias_token_ranges: List[Tuple[int, int, str, str]] = (
        []
    )  # (start, end, text, tenant_service_id)
    ...
    for span in alias_spans:
        ...
        is_modifier = (
            canonical_family in canonical_families_from_services and tenant_aliases
        )
        ...
        if is_modifier:
            ...
            alias_token_ranges.append(
                (start_tok, end_tok, span["text"], tenant_service_id)
            )
    ...
    for service in services:
        ...
        entity_text = service.get("text", "").lower()
        canonical_family = service.get("canonical") or business_category_map.get(
            entity_text
        )
...
def consume_service_annotations(
    doc, annotations: Dict[str, Any], logger_instance: Optional[logging.Logger] = None
