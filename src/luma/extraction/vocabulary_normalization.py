...
def load_vocabularies(global_json_path: Path) -> Dict[str, Dict[str, Dict[str, Any]]]:
    ...
    vocabularies = (
        data.get("normalization", {}).get("normalization", {}).get("vocabularies", {})
    )
    return vocabularies
...
def normalize_vocabularies(
    text: str, synonym_map: Dict[str, str], typo_map: Dict[str, str]
) -> Tuple[str, bool]:
    """
...
def validate_vocabularies(
    ...
    all_variants: Dict[str, Tuple[str, str]] = (
        {}
    )  # variant -> (category, type: synonym|typo)
                ...
                for synonym in synonyms:
                    if isinstance(synonym, str):
                        ...
                        if synonym_lower in all_variants:
                            existing_category, existing_type = all_variants[
                                synonym_lower
                            ]
                            errors.append(
