...
def run_test_case(
    ...
    try:
        ...
        if should_extract:
            assert (
                len(times) > 0
            ), f"Expected time extraction for '{test_input}', but got no times"
            assert (
                has_timetoken
            ), f"Expected timetoken in psentence for '{test_input}', but psentence='{psentence}'"
            print("✅ PASS: Time extracted correctly")
        else:
            assert (
                len(times) == 0
            ), f"Expected no time extraction for '{test_input}', but got times: {times}"
            assert (
                not has_timetoken
            ), f"Expected no timetoken for '{test_input}', but psentence='{psentence}'"
...
def main():
    ...
    run_test_case(
        matcher, "14 : 00", "24-hour format with spaces: '14 : 00'", should_extract=True
    )
    run_test_case(
        matcher, "14:00", "24-hour format without spaces: '14:00'", should_extract=True
    )
    run_test_case(
        matcher,
        "09 : 00",
        "24-hour format with leading zero and spaces: '09 : 00'",
        should_extract=True,
    )
    run_test_case(
        matcher,
        "18 : 30",
        "24-hour format with minutes and spaces: '18 : 30'",
        should_extract=True,
    )
    run_test_case(
        matcher, "18:30", "24-hour format with minutes: '18:30'", should_extract=True
    )
    ...
    run_test_case(
        matcher,
        "9 : 30 am",
        "12-hour format with spaces: '9 : 30 am'",
        should_extract=True,
    )
    run_test_case(
        matcher,
        "9:30am",
        "12-hour format without spaces: '9:30am'",
        should_extract=True,
    )
    run_test_case(
        matcher,
        "12 : 00 pm",
        "12-hour format noon with spaces: '12 : 00 pm'",
        should_extract=True,
    )
    ...
    run_test_case(
        matcher,
        "14 :",
        "Incomplete time (missing minutes): '14 :'",
        should_extract=False,
    )
    run_test_case(
        matcher, ": 00", "Incomplete time (missing hour): ': 00'", should_extract=False
    )
    run_test_case(
        matcher, "14", "Bare number without colon: '14'", should_extract=False
    )
    ...
    run_test_case(
        matcher, "before 4 pm", "Time constraint: 'before 4 pm'", should_extract=True
    )
    run_test_case(matcher, "by 6 pm", "Time constraint: 'by 6 pm'", should_extract=True)
    run_test_case(
        matcher, "after 10 am", "Time constraint: 'after 10 am'", should_extract=True
    )
    ...
    run_test_case(
        matcher,
        "book me for facial at 14 : 00",
        "Time in booking context: 'book me for facial at 14 : 00'",
        should_extract=True,
    )
    run_test_case(
        matcher,
        "make it 18 : 30",
        "Time in modification context: 'make it 18 : 30'",
        should_extract=True,
    )
