...
def test_alias_phrase_creates_service_entity():
    ...
    config_data_path = (
        Path(__file__).parent.parent / "config" / "data" / "global.v2.json"
    )
    store_path = (
        Path(__file__).parent.parent / "store" / "normalization" / "global.v2.json"
    )
    ...
    # Tenant aliases
    tenant_aliases = {"premium haircut": "beauty_and_wellness.haircut"}
    ...
    result = matcher.extract_with_parameterization(
        text=text, tenant_aliases=tenant_aliases
    )

    # Should have service_family entity from alias
    service_families = result.get("business_categories") or result.get(
        "service_families", []
    )
    ...
    ), "Should have service_family entity with canonical 'beauty_and_wellness.haircut'"
    assert (
        alias_entity["canonical"] == "beauty_and_wellness.haircut"
    ...
    assert (
        "servicefamilytoken" in psentence
...
def test_alias_no_match_when_phrase_absent():
    ...
    if alias_entity_found:
        ...
        print(
            "Note: Service entity found, but it's from explicit canonical (not alias phrase)"
        )
...
def test_explicit_service_overrides_alias():
    ...
    tenant_aliases = {"premium service": "beauty_and_wellness.haircut"}
    ...
    assert (
        explicit_entity is not None or len(service_families) > 0
