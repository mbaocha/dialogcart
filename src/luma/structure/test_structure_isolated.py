...
spec_types = importlib.util.spec_from_file_location(
    "luma.structure.structure_types", types_path
)
...
spec_interpreter = importlib.util.spec_from_file_location(
    "luma.structure.interpreter", interpreter_path
)
...
def test_shared_vs_separate_services():
    ...
    assert (
        result1.service_scope == "shared"
    ), f"Expected 'shared', got '{result1.service_scope}'"
    ...
    assert (
        result2.service_scope == "separate"
    ), f"Expected 'separate', got '{result2.service_scope}'"
...
def test_exact_vs_range_time():
    ...
    assert (
        result3.time_type == "window"
    ), f"Expected 'window', got '{result3.time_type}'"
...
def test_shared_vs_per_service_time():
    ...
    result1 = interpret_structure(psentence1, entities1)
    assert (
        result1.time_scope == "shared"
    ), f"Expected 'shared', got '{result1.time_scope}'"
    ...
    result2 = interpret_structure(psentence2, entities2)
    assert (
        result2.time_scope == "per_service"
    ), f"Expected 'per_service', got '{result2.time_scope}'"
...
def test_clarification_needed():
    ...
    result1 = interpret_structure(psentence1, entities1)
    assert (
        result1.needs_clarification is True
    ), "Should need clarification for multiple dates"
    ...
    result2 = interpret_structure(psentence2, entities2)
    assert (
        result2.needs_clarification is True
    ), "Should need clarification for multiple times"
