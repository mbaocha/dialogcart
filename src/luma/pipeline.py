...
from luma.decision import DecisionResult, decide_booking_status
...
from luma.trace import create_slot_snapshot, log_slot_transformation
...
PIPELINE_STAGE_CONTRACTS = {
    ...
    "grouping": {
        ...
        "allowed_inputs": {
            "extraction",
            "structure",
        },  # May read extraction and structure
        "forbidden": {
            "must_not_read": {"intent", "semantic", "decision", "calendar"},
            "must_not_mutate": [
                "extraction",
                "structure",
            ],  # Must not modify upstream results
        },
    },
    "semantic": {
        "outputs": {
            "required": [
                "resolved_booking"
            ],  # Contains: services, date_mode, date_refs, time_mode, time_refs, etc.
            "optional": ["needs_clarification", "clarification"],
        },
        "allowed_inputs": {
            "extraction",
            "grouping",
        },  # May read extraction and grouping
        "forbidden": {
    ...
    "decision": {
        ...
        "allowed_inputs": {
            "extraction",
            "semantic",
        },  # May read extraction and semantic
        "forbidden": {
    ...
    "calendar": {
        ...
        "allowed_inputs": {
            ...
            "semantic",
            "intent",
        },  # May read extraction, semantic, and intent
        "forbidden": {
...
def _validate_stage_boundary(
    ...
    if stage_name == "extraction":
        if not isinstance(stage_output, dict):
            ...
            for output_key in required_outputs:
                if output_key not in stage_output:
                    errors.append(
                        f"Stage 'extraction' missing required output: {output_key}"
                    )
        ...
        if not isinstance(stage_output, dict):
            ...
            for output_key in required_outputs:
                if output_key not in stage_output:
                    errors.append(
                        f"Stage 'intent' missing required output: {output_key}"
                    )
        ...
        if not hasattr(stage_output, "to_dict"):
            errors.append(
                f"Stage 'structure' must return StructureResult with to_dict() method"
            )
        else:
            ...
            for output_key in required_outputs:
                if output_key not in structure_dict:
                    errors.append(
                        f"Stage 'structure' missing required output: {output_key}"
                    )
        ...
        if not isinstance(stage_output, dict):
            ...
            for output_key in required_outputs:
                if output_key not in stage_output:
                    errors.append(
                        f"Stage 'grouping' missing required output: {output_key}"
                    )
            ...
            if not isinstance(resolved_booking, dict):
                ...
                for key in required_in_booking:
                    if key not in resolved_booking:
                        errors.append(
                            f"Stage 'semantic' resolved_booking missing required field: {key}"
                        )
        ...
        if not isinstance(stage_output, DecisionResult):
            ...
            if not hasattr(stage_output, "status"):
                errors.append(
                    f"Stage 'decision' DecisionResult missing required attribute: status"
                )
        ...
        if not isinstance(stage_output, CalendarBindingResult):
            ...
            if not hasattr(stage_output, "calendar_booking"):
                errors.append(
                    f"Stage 'calendar' CalendarBindingResult missing required attribute: calendar_booking"
                )
    ...
    if errors:
        error_msg = (
            f"Pipeline stage contract violation for '{stage_name}':\n"
...
class LumaPipeline:
    ...
    def run(
        ...
        """
        results = {"stages": {}, "execution_trace": {}}
        ...
        with StageTimer(
            results["execution_trace"], "extraction", request_id=request_id
        ):
            matcher = get_entity_matcher(
                domain=self.domain, entity_file=self.entity_file
            )
            extraction_result = matcher.extract_with_parameterization(
                text,
                request_id=request_id,
                tenant_aliases=(
                    tenant_context.get("aliases") if tenant_context else None
                ),
            )
        ...
        assert (
            "psentence" in extraction_result
        ), "extraction_result must contain 'psentence' key"
        assert isinstance(
            extraction_result.get("psentence"), str
        ), "psentence must be a string"

        # Contract validation: extraction stage output
        previous_stages = {}
        _validate_stage_boundary(
            "extraction", extraction_result, previous_stages, debug_mode=debug_mode
        )
        ...
        _validate_stage_boundary(
            "intent", intent_resp, previous_stages, debug_mode=debug_mode
        )
        ...
        assert isinstance(
            structure_dict, dict
        ), "structure.to_dict() must return a dict"
        assert (
            "structure" in structure_dict
        ), "structure.to_dict() must contain 'structure' key"

        # Contract validation: structure stage output
        _validate_stage_boundary(
            "structure", structure, previous_stages, debug_mode=debug_mode
        )
        ...
        if config.LOG_SLOT_TRACKING:
            log_slot_transformation(
                "structure",
                extraction_result,
                structure_dict.get("structure", {}),
                request_id=request_id,
                enabled=config.LOG_SLOT_TRACKING,
            )
            snapshot = create_slot_snapshot(
                structure_dict.get("structure", {}), "structure"
            )
            if "slot_snapshots" not in results["execution_trace"]:
                results["execution_trace"]["slot_snapshots"] = []
            results["execution_trace"]["slot_snapshots"].append(snapshot)
        ...
        _validate_stage_boundary(
            "grouping", grouped_result, previous_stages, debug_mode=debug_mode
        )
        ...
        ), "semantic_result must be a SemanticResolutionResult"
        assert isinstance(
            semantic_result.resolved_booking, dict
        ), "semantic_result.resolved_booking must be a dict"
        ...
        _validate_stage_boundary(
            "semantic", semantic_result, previous_stages, debug_mode=debug_mode
        )
        ...
        assert decision_result.status in (
            "RESOLVED",
            "NEEDS_CLARIFICATION",
        ), f"decision_result.status must be 'RESOLVED' or 'NEEDS_CLARIFICATION', got '{decision_result.status}'"
        # Invariant: decision_trace is a dict with decision key
        assert isinstance(decision_trace, dict), "decision_trace must be a dict"
        assert (
            "decision" in decision_trace
        ), "decision_trace must contain 'decision' key"
        assert isinstance(
            decision_trace["decision"], dict
        ), "decision_trace['decision'] must be a dict"

        # Contract validation: decision stage output
        _validate_stage_boundary(
            "decision", decision_result, previous_stages, debug_mode=debug_mode
        )
        ...
        decision_missing_slots = decision_trace.get("decision", {}).get(
            "missing_slots", []
        )
        has_date = bool(semantic_result.resolved_booking.get("date_refs"))
        missing_only_time = (
            decision_result.status == "NEEDS_CLARIFICATION"
            and decision_result.reason == "temporal_shape_not_satisfied"
            and len(decision_missing_slots) == 1
            and decision_missing_slots == ["time"]
            and has_date
        )
        ...
        if decision_result.status == "RESOLVED" or missing_only_time:
            ...
            external_intent = (
                None  # Not needed - intent_name is already the real intent
            )

            with StageTimer(
                results["execution_trace"], "binder", request_id=request_id
            ):
            ...
            ), "calendar_result must be a CalendarBindingResult"
            ...
            assert isinstance(
                binder_trace["binder"], dict
            ), "binder_trace['binder'] must be a dict"

            # Contract validation: calendar stage output
            _validate_stage_boundary(
                "calendar", calendar_result, previous_stages, debug_mode=debug_mode
            )
            ...
            results["execution_trace"]["binder"] = {
                ...
                "input": {
                    "intent": intent_name,
                    "external_intent": external_intent,
                    "date_mode": semantic_result.resolved_booking.get(
                        "date_mode", "none"
                    ),
                    "date_refs": semantic_result.resolved_booking.get("date_refs", []),
                    "time_mode": semantic_result.resolved_booking.get(
                        "time_mode", "none"
                    ),
                    "time_refs": semantic_result.resolved_booking.get("time_refs", []),
                    "time_constraint": semantic_result.resolved_booking.get(
                        "time_constraint"
                    ),
                    "timezone": timezone,
                },
            ...
            assert (
                "binder" in results["execution_trace"]
            ), "execution_trace must contain 'binder' key when calendar binding is skipped"
            assert (
                results["execution_trace"]["binder"]["called"] is False
            ), "binder.called must be False when calendar binding is skipped"
            ...
            if config.LOG_SLOT_TRACKING:
                snapshot = create_slot_snapshot(
                    calendar_result, "calendar_binding_skipped"
                )
                if "slot_snapshots" not in results["execution_trace"]:
                    results["execution_trace"]["slot_snapshots"] = []
                results["execution_trace"]["slot_snapshots"].append(snapshot)
        ...
        assert (
            "binder" in results["execution_trace"]
        ), "execution_trace must always contain 'binder' key"
        assert isinstance(
            results["execution_trace"]["binder"], dict
        ), "execution_trace['binder'] must be a dict"
        assert (
            "called" in results["execution_trace"]["binder"]
        ), "execution_trace['binder'] must contain 'called' key"
        assert isinstance(
            results["execution_trace"]["binder"]["called"], bool
        ), "execution_trace['binder']['called'] must be a bool"
