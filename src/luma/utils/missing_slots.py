...
def derive_missing_slot_from_reason(temporal_shape_reason: Optional[str]) -> List[str]:
    ...
    missing_slot = (
        temporal_shape_reason.lower().replace("missing_", "").replace("_", "_")
    )
...
def compute_missing_slots_for_intent(
    ...
    enforced_missing = validate_required_slots(intent_name, resolved_slots, entities)
    ...
    if intent_name == "MODIFY_BOOKING":
        ...
        if booking_id_present:
            ...
            has_date_in_entities = bool(
                entities.get("dates") or entities.get("dates_absolute")
            )
            ...
            if (
                resolved_slots.get("service_id")
                or resolved_slots.get("services")
                or entities.get("service_id")
                or entities.get("business_categories")
                or entities.get("service_families")
            ):
                has_delta = True

            # Check duration delta (check both resolved_slots and entities)
            if resolved_slots.get("duration") or entities.get("durations"):
                has_delta = True
            ...
            if not has_delta:
                ...
                enforced_missing = [
                    slot for slot in enforced_missing if slot != "booking_id"
                ]
