...
def validate_stable_fields(
    execution_trace_data: Dict[str, Any], debug_mode: bool = False
) -> Optional[str]:
    """
            ...
            for field in STABLE_RESPONSE_FIELDS:
                if field not in response:
                    ...
                    if field == "status" and value not in (
                        ...
                        errors.append(
                            f"Stable field 'trace.response.status' must be '{STATUS_READY}' or '{STATUS_NEEDS_CLARIFICATION}', got '{value}'"
                        )
                    elif field == "intent" and not isinstance(value, str):
                        errors.append(
                            f"Stable field 'trace.response.intent' must be a string"
                        )
                    elif field == "issues" and not isinstance(value, dict):
                        errors.append(
                            f"Stable field 'trace.response.issues' must be a dict"
                        )
                    elif field in (
                        "has_booking",
                        "has_clarification",
                        ...
                        errors.append(
                            f"Stable field 'trace.response.{field}' must be a bool"
                        )
            ...
            for field in STABLE_SEMANTIC_FIELDS:
                if field not in semantic:
                    ...
                    if field == "service_ids" and not isinstance(value, list):
                        errors.append(
                            f"Stable field 'trace.semantic.service_ids' must be a list"
                        )
                    elif field in ("date_mode", "time_mode") and not isinstance(
                        value, str
                    ):
                        errors.append(
                            f"Stable field 'trace.semantic.{field}' must be a string"
                        )
                    elif field in ("date_refs", "time_refs") and not isinstance(
                        value, list
                    ):
                        errors.append(
                            f"Stable field 'trace.semantic.{field}' must be a list"
                        )
                    elif field == "needs_clarification" and not isinstance(value, bool):
                        errors.append(
                            f"Stable field 'trace.semantic.needs_clarification' must be a bool"
                        )
                    elif (
                        ...
                        errors.append(
                            f"Stable field 'trace.semantic.clarification_reason' must be a string or None"
                        )
                    # time_constraint is optional (may be None)
                        ...
                        errors.append(
                            f"Stable field 'trace.semantic.time_constraint' must be a dict or None"
                        )
        ...
        if isinstance(decision, dict):
            for field in STABLE_DECISION_FIELDS:
                if field not in decision:
                    ...
                    if field == "state" and value not in (
                        "RESOLVED",
                        "NEEDS_CLARIFICATION",
                    ):
                        errors.append(
                            f"Stable field 'trace.decision.state' must be 'RESOLVED' or 'NEEDS_CLARIFICATION', got '{value}'"
                        )
                    elif (
                        field == "reason"
                        and value is not None
                        ...
                        errors.append(
                            f"Stable field 'trace.decision.reason' must be a string or None"
                        )
                    elif field == "missing_slots" and not isinstance(value, list):
                        errors.append(
                            f"Stable field 'trace.decision.missing_slots' must be a list"
                        )
                    elif (
                        field in ("expected_temporal_shape", "actual_temporal_shape")
                        and value is not None
                        and not isinstance(value, str)
                    ):
                        errors.append(
                            f"Stable field 'trace.decision.{field}' must be a string or None"
                        )
                    elif field == "temporal_shape_satisfied" and not isinstance(
                        value, bool
                    ):
                        errors.append(
                            f"Stable field 'trace.decision.temporal_shape_satisfied' must be a bool"
                        )
...
def _format_errors(errors: list, debug_mode: bool) -> Optional[str]:
    ...
    error_msg = (
        f"EXECUTION_TRACE stable field validation failed (version {TRACE_VERSION}):\n"
