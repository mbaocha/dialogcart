...
def compute_slot_diff(
    before: Dict[str, Any], after: Dict[str, Any], slot_keys: Optional[Set[str]] = None
) -> Dict[str, Any]:
    """
...
def _sanitize_for_log(value: Any, max_length: int = 200) -> Any:
    ...
    if isinstance(value, dict):
        ...
        if len(value) > 20:
            return {
                k: _sanitize_for_log(v, max_length) for k, v in list(value.items())[:20]
            }
        ...
        if diff["slots_added"] or diff["slots_removed"] or diff["slots_modified"]:
            logger.debug(
                ...
                extra={
                    ...
                    "slot_transformation": {
                        "input_slots": (
                            list(input_slots.keys())
                            if isinstance(input_slots, dict)
                            else []
                        ),
                        "output_slots": (
                            list(output_slots.keys())
...
def create_slot_snapshot(data: Any, stage_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"stage": stage_name, "slots": _sanitize_for_log(slots, max_length=500)}
    except Exception as e:  # noqa: BLE001
...
def _extract_slot_data(data: Any) -> Dict[str, Any]:
    ...
    if isinstance(data, dict):
        ...
        return {
            k: v for k, v in data.items() if k in slot_keys or not k.startswith("_")
        }
