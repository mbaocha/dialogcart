...
def profile_pipeline(
    pipeline: LumaPipeline, text: str, tenant_context: Dict[str, Any] = None
) -> pstats.Stats:
...
def identify_bottlenecks(
    timing_data: List[Dict[str, Any]], profile_data: List[Dict[str, Any]] = None
) -> tuple:
    ...
    func_aggregates = defaultdict(
        lambda: {"total_time": 0, "call_count": 0, "cumulative_time": 0}
    )
    ...
    for name, agg in sorted(
        func_aggregates.items(), key=lambda x: x[1]["cumulative_time"], reverse=True
    )[:20]:
    ...
    for name, agg in sorted(
        func_aggregates.items(), key=lambda x: x[1]["call_count"], reverse=True
    )[:20]:
            ...
            bottlenecks["frequent_calls"].append(
                {
                    ...
                    "avg_time_per_call_ms": round(
                        (agg["total_time"] * 1000) / agg["call_count"], 4
                    ),
                }
            )
...
def generate_report(
    ...
    if not recommendations:
        recommendations.append(
            "• No major bottlenecks identified. Performance looks good!"
        )
...
def main():
    ...
    for i, scenario in enumerate(all_scenarios, 1):
        ...
        timing_result = run_pipeline_with_timing(
            pipeline, text, tenant_context, iterations=args.iterations
        )
    ...
    bottlenecks, avg_stages = identify_bottlenecks(
        timing_data, profile_data if args.profile else []
    )
