...
def get_entity_matcher(
    domain: str, entity_file: Optional[str] = None, lazy_load_spacy: bool = False
) -> EntityMatcher:
    """
    ...
    if lazy_load_spacy:
        return EntityMatcher(
            domain=domain, entity_file=entity_file, lazy_load_spacy=True
        )
    ...
    with _cache_lock:
        ...
        matcher = EntityMatcher(
            domain=domain, entity_file=entity_file, lazy_load_spacy=False
        )
