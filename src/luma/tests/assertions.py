...
from luma.config.core import STATUS_NEEDS_CLARIFICATION, STATUS_READY
...
def assert_clarification_has_missing_slots(response: Dict[str, Any]) -> None:
    ...
    missing_slots = [
        slot
        for slot, issue in issues.items()
        if issue == "missing"
        or (isinstance(issue, dict) and issue.get("type") == "missing")
    ]
...
def assert_ready_has_required_bound_fields(
    response: Dict[str, Any], intent_name: Optional[str] = None
) -> None:
    """
...
def assert_status_missing_slots_consistency(response: Dict[str, Any]) -> None:
    ...
    if status == STATUS_READY:
        ...
        has_missing_string_issues = any(issue == "missing" for issue in issues.values())
        ...
        has_any_issue = (
            len(issues) > 0
        )  # Even non-missing issues (like ambiguous_meridiem)
...
def assert_booking_block_consistency(
    response: Dict[str, Any], intent_name: Optional[str] = None
) -> None:
    """
