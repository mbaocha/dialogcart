...
"""
DynamoDB Table: agentic_catalog

Primary Key (composite):
    - PK (string) → TENANT#{tenant_id}
    - SK (string) → PRODUCT#{product_id} | VARIANT#{variant_id}

Items:
    - PRODUCT items (reference/metadata per product)
    - VARIANT items (one per purchasable SKU; denormalized for fast browse)

Common Attributes:
    - entity (string)            → "PRODUCT" or "VARIANT"
    - tenant_id (string)
    - product_id (string)
    - handle (string)            → SEO slug
    - title (string)             → product title (shared text for prefix search)
    - vendor (string, optional)
    - tags (list<string>)
    - collections (list<string>)
    - category (map)             → { name, path[], source, confidence }   # generic
    - category_name (string)     → duplicated for indexing
...
Variant-only Attributes:
    ...
    - sku (string, optional)
    - options (map)              → e.g., {"Size":"OS","Color":"black"}
    - price_num (number)
    - price_sort (string)        → zero-padded for lexical sort, e.g. "00000070.00"
    - compare_at_num (number, optional)
...
Global Secondary Indexes (GSIs):
    ...
    - GSI4_TitlePrefix     → Tenant-scoped title prefix search (typeahead)
        ...
        SK: GSI4SK = <normalized_lower_title>

Indexing strategy:
    - We only set GSI1/2/3 attributes when in_stock = True (sparse indexes).
      When stock drops to 0, we remove those attributes so items disappear from browse GSIs.
"""
...
def zero_pad_price(price: float, width: int = 11, decimals: int = 2) -> str:
    # e.g., 70 → "00000070.00" so lexical sort works numerically
...
class CatalogDB:
    def __init__(self, table_name: str = "agentic_catalog", region_name: str = "eu-west-2"):
        self.table = boto3.resource("dynamodb", region_name=region_name).Table(table_name)

    # ----------- Product CRUD -----------

    def put_product(self, tenant_id: str, product: Dict[str, Any]) -> None:
        """
        ...
        """
        item = dict(product)  # shallow copy
        item["PK"] = pk_tenant(tenant_id)
        item["SK"] = sk_product(item["product_id"])
        item["entity"] = "PRODUCT"
        if "category" in item and item["category"]:
            item["category_name"] = item["category"].get("name")
        self.table.put_item(Item=item)

    def get_product(self, tenant_id: str, product_id: str) -> Optional[Dict[str, Any]]:
        resp = self.table.get_item(Key={"PK": pk_tenant(tenant_id), "SK": sk_product(product_id)})
    ...
    def delete_product_and_variants(self, tenant_id: str, product_id: str) -> int:
        """
        ...
        deleted = 0
        # delete product
        prod_key = {"PK": pk_tenant(tenant_id), "SK": sk_product(product_id)}
        try:
        ...
        while True:
            kwargs = {
                ...
                "FilterExpression": Attr("entity").eq("VARIANT") & Attr("product_id").eq(str(product_id)),
    ...
    def list_variants_for_product(self, tenant_id: str, product_id: str, limit: int = 100,
                                  last_key: Optional[Dict[str, Any]] = None) -> Tuple[List[Dict[str, Any]], Optional[Dict[str, Any]]]:
        """
