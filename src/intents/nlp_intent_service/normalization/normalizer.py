...
class SimpleTextNormalizer(GraphComponent):
    ...
    # ✅ FIXED: must match Rasa component interface
    @classmethod
    def create(
        cls,
        ...
        resource: Resource,
        execution_context: ExecutionContext,  # 👈 REQUIRED for Rasa 3.x+
    ...
    def _load_maps(
        ...
        if config_file:
            ...
            if not os.path.exists(path):
                raise FileNotFoundError(f"SimpleTextNormalizer: config_file not found: {path}")
            ...
            logger.info("📄 Loaded normalization maps from %s", path)
        else:
